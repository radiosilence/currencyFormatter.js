...
export type Currency = keyof typeof defaultLocales;

export type EncodedPattern = {
  pattern: string;
  decimal: string;
...
type Preferences = {
  currency?: Currency; // If currency is not supplied, defaults to USD
  symbol?: string; // Overrides the currency's default symbol
  locale?: string; // Overrides the currency's default locale (see supported locales)
  decimal?: string; // Overrides the locale's decimal character
  group?: string; // Overrides the locale's group character (thousand separator)
  pattern?: string; // Overr
  valueOnError?: string;
  postFormatFunction?: (n: number, s: string) => string;
  selector?: string;
};

export const getFormatDetails = function (p: Preferences = {}) {
  // Perform checks on inputs and set up defaults as needed (defaults to en, USD)

  const currency: Currency = p.currency || 'USD';
  ...
  const valueOnError =
    typeof p.valueOnError === 'undefined' ? 0 : p.valueOnError;
...
export const toFixed = function (n: number, precision: number) {
  return (
    Math.round(Number(n) * Math.pow(10, precision)) / Math.pow(10, precision)
  ).toFixed(precision);
};

export function getFormatter(p: Preferences) {
  ...
  const pad = function (input: string | number, width: number) {
    const n = `${input}`;
  ...
  function format(n: number, f: EncodedPattern) {
    ...
    if (maxGroupIndex > 0) {
      while (cursor > 0) {
        ...
        segment = `${splitNumber[0].substring(start, cursor)}${
          f.group
        }${segment}`;
    ...
    const formatted = `${f.frontPadding}${segment}${
      splitNumber[1] === undefined ? '' : `${f.decimal}${splitNumber[1]}`
    }${f.backPadding}`;
  ...
  const positiveFormat = encodePattern(patternArray[0]) as EncodedPattern;
  ...
  const negativeFormat = (patternArray[1] === undefined
    ...
    : encodePattern(patternArray[1])) as EncodedPattern;
  ...
  const zero =
    patternArray[2] === undefined ? format(0, positiveFormat) : patternArray[2];
...
export function format(n: number, p: Preferences) {
  const formatterFunction = getFormatter(p);
  return formatterFunction(n);
}

export function parse(str: string, p: Preferences) {
